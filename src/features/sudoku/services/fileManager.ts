...
export class FileManager {
  ...
  static saveGrid(grid: SudokuGrid, solution?: readonly (readonly number[])[]): string {
  ...
  static loadGrid(jsonData: string): SudokuGrid {
    return this.loadGame(jsonData).grid;
  }

  static loadGame(jsonData: string): { grid: SudokuGrid; solution?: readonly (readonly number[])[] } {
    try {
      ...
      if (saveData.solution && !this.isValidSolution(saveData.solution)) {
  ...
  private static isValidGridStructure(grid: unknown): grid is SudokuGrid {
    if (!this.isRecord(grid) || !Array.isArray(grid.cells)) {
      return false;
    }
  ...
  private static isValidCell(cell: unknown, expectedRow: number, expectedCol: number): boolean {
    return (
      this.isRecord(cell) &&
      cell.row === expectedRow &&
      cell.col === expectedCol &&
      (cell.value === null || (typeof cell.value === 'number' && cell.value >= 1 && cell.value <= 9)) &&
      typeof cell.isGiven === 'boolean' &&
  ...
  private static isValidSolution(solution: unknown): solution is readonly (readonly number[])[] {
    return (

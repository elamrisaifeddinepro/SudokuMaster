...
export const NotationPanel: React.FC<NotationPanelProps> = ({
  ...
  const getModeLabel = (modeType: NotationMode) => {
    switch (modeType) {
      case 'value': return t('notation.value');
      case 'corner': return t('notation.corner');
      case 'center': return t('notation.center');
      case 'color': return t('notation.color');
    }
  };

  return (
    <div className="app-surface rounded-2xl p-6 shadow-lg border border-slate-700 space-y-4">
      <h3 className="text-lg font-bold text-slate-100">{t('notation.title')}</h3>
      ...
      {mode !== 'color' && (
        <div>
          <h4 className="text-sm font-semibold text-slate-300 mb-2">{t('notation.numbers')}</h4>
      ...
      {mode === 'color' && (
        <div>
          <h4 className="text-sm font-semibold text-slate-300 mb-2">{t('notation.colors')}</h4>
          <div className="grid grid-cols-3 gap-2">
            {colors.map(color => (
              <button
                key={color}
      ...
      >
        {mode === 'color' ? t('notation.removeColor') : t('notation.clear')}
      </button>
    </div>
  );
};
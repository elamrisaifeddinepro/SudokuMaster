...
export const LeaderboardModal: React.FC<LeaderboardModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="modal-pop app-surface rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden border border-slate-700">
        {/* Header */}
        ...
        <div className="p-6 border-b border-slate-700">
          {!isSupabaseConfigured && (
            <div className="mt-4 rounded-xl border border-amber-400/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
              Le tableau d’honneur en ligne n’est pas configuré pour cette version locale. Ajoutez les variables Supabase dans <code className="text-amber-50">.env</code> pour activer la sauvegarde des scores.
            </div>
          )}
            ...
            <div className="text-center py-12">
              ...
              <div className="mt-4 p-4 bg-cyan-500/10 border border-cyan-400/30 rounded-lg">
                <p className="text-sm text-cyan-100 mb-3">
                  📋 Pour activer le tableau d'honneur, configurez Supabase dans le fichier .env :
                </p>
                <div className="space-y-2 text-xs text-cyan-200">
                  <p>1. Copiez .env.example vers .env</p>
                  <p>2. Ajoutez VITE_SUPABASE_URL et VITE_SUPABASE_ANON_KEY</p>
                  <p>3. Exécutez la migration SQL fournie dans supabase/migrations</p>
